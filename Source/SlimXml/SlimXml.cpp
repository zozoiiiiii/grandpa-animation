...
void XmlBase::assignString(Char* &str, Char* buffer, size_t length, bool transferCharacter)
{
	...
	if (!transferCharacter || (found = Memchr(buffer, SXT('&'), length)) == NULL)
	{
		str = buffer;
		str[length] = 0;
		return;
	...
	String temp;
	for (; found != NULL; found = Memchr(buffer, SXT('&'), length))
	{
		...
		if (length >= 5)
		{
			if (Strncmp(buffer, SXT("quot"), 4) == 0)
			{
				buffer += 4;
				length -= 4;
				temp.append(1, SXT('\"'));
			...
			if (Strncmp(found + 1, SXT("apos"), 4) == 0)
			{
				buffer += 4;
				length -= 4;
				temp.append(1, SXT('\''));
		...
		if (length >= 3)
		{
			...
			else if (Strncmp(buffer, SXT("gt"), 2) == 0)
			{
				buffer += 2;
				length -= 2;
				temp.append(1, SXT('>'));
...
void XmlNode::readAttributeAsIntArray(const Char* name, int* out, unsigned long length, int defaultValue) const
{
	...
	unsigned long stringLen = Strlen(attribute->getValue<const Char*>());
	Char* tempBuffer = new Char[stringLen+1];
	Strcpy(tempBuffer, attribute->getValue<const Char*>());
	Char* current = tempBuffer;

	unsigned long index = 0;
	while (index < length)
	{
		Char* found = (Char*)Memchr(current, SXT(','), stringLen);
		if (found != NULL)
		{
			*found = 0;
		}
										   ...
										   unsigned long enumCount, unsigned long defaultValue) const
{
	...
	{
		if (Strcmp(enumNames[i], attribute->getValue<const Char*>()) == 0)
		{
			return i;
		}
	}
...
void XmlNode::writeNode(String& output, int depth) const
{
		  ...
		  ++iter)
	{
		...
		output += SXT("=\"");
		writeTransferredString(output, attribute->getValue<const Char*>());
		output += SXT('"');
	}
...
bool XmlDocument::save(const Char* filename, Encode encode) const
{
	...
	{
		output = SXT("<?xml version=\"1.0\" encoding=\"UTF-16\"?>\r\n");
	}
	else if (encode == UTF_8 || encode == UTF_8_NO_MARK)
	{
		output = SXT("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
	}
#elif defined (UNICODE)
...
bool XmlDocument::parse(Char* input, size_t size)
{
	...
	while (cur < end)
	{
		...
		case SXT('/'):	//node ending
			...
			--depth;
			break;
		case SXT('?'):	//xml define node, ignore
			break;
		case SXT('!'):	//comment node
			{
				//ignore !-- and --
				if (labelSize < 5)
				{
...
{
	label = (Char*)Memchr(begin, SXT('<'), size);
	if (label == NULL)
	{
		return false;
	}
		...
		&& label[2] == SXT('-'))
	{
		...
		while (true)
		{
			Char* end = (Char*)Memchr(cur, SXT('-'), size);
			if (end == NULL)
			{
				return false;
			}
			if (*(end+1) == SXT('-') && *(end+2) == SXT('>'))
			{
				//get it
	...
	begin = (Char*)Memchr(label, SXT('>'), size);
	if (begin == NULL)
	{
		return false;
	}
...
bool XmlDocument::parseLabel(XmlNode* node, Char* label, size_t labelSize)
{
	...
	while (cur < end)
	{
		...
		//attribute name
		Char* attrName = cur;
		while (*cur != SXT(' ') && *cur != SXT('=') && *cur != SXT('/') && *cur != SXT('>'))
		{
			++cur;
		}
		next = *cur;
		...
		//attribute value
		cur = (Char*)Memchr(cur, SXT('"'), end - cur);
		if (cur == NULL)
		{
			break;
		}
